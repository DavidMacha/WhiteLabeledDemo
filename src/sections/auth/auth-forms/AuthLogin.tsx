...
import { useAuth } from './AuthContext'; // Use your custom AuthContext

const AuthLogin = () => {
  const scriptedRef = useScriptRef();
  const [checked, setChecked] = useState(false);
  const [showPassword, setShowPassword] = useState(false);
  const { setAuthData } = useAuth(); // Set auth data in context
    ...
    <Formik
      ...
      onSubmit={async (values, { setErrors, setStatus, setSubmitting }) => {
        try {
          ...
          // Handle the response from API and store the user data
          if (response.status === 200 && response.data.status === 'treatmentSuccess') {
            setStatus({ success: true });
            setSubmitting(false);

            // Save the API response (user info, token, etc.) to auth context
            setAuthData(response.data); // Store response in context or local storage
